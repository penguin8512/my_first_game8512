...
font = (
    pygame.font.Font("fonts/msjh.ttf", 30)
    if pygame.font.get_init()
    else pygame.font.SysFont("Arial", 30)
)
...
try:
    # 【主選單按鈕圖片】
    btn1_normal = pygame.transform.scale(
        pygame.image.load("images/button/btn1-1.png").convert_alpha(), (600, 300)
    )
    btn1_hover = pygame.transform.scale(
        pygame.image.load("images/button/btn1-2.png").convert_alpha(), (600, 300)
    )
    btn1_pressed = pygame.transform.scale(
        pygame.image.load("images/button/btn1-3.png").convert_alpha(), (600, 300)
    )

    btn2_normal = pygame.transform.scale(
        pygame.image.load("images/button/btn2-1.png").convert_alpha(), (600, 300)
    )
    btn2_hover = pygame.transform.scale(
        pygame.image.load("images/button/btn2-2.png").convert_alpha(), (600, 300)
    )
    btn2_pressed = pygame.transform.scale(
        pygame.image.load("images/button/btn2-3.png").convert_alpha(), (600, 300)
    )

    btn3_normal = pygame.transform.scale(
        pygame.image.load("images/button/btn3-1.png").convert_alpha(), (600, 300)
    )
    btn3_hover = pygame.transform.scale(
        pygame.image.load("images/button/btn3-2.png").convert_alpha(), (600, 300)
    )
    btn3_pressed = pygame.transform.scale(
        pygame.image.load("images/button/btn3-3.png").convert_alpha(), (600, 300)
    )
...
def draw_robot_ui(bot_text, show_input=False, input_val=""):
    ...
    if show_input:
        ...
        screen.blit(
            text_surf, (box_rect.x + 15, box_rect.centery - text_surf.get_height() // 2)
        )

        hint = font.render(
            "請輸入您的英文名字，完成後請按 ENTER", True, (150, 150, 150)
        )
...
while running:
    ...
    if game_state == "chatbot":
        if chatbot_step == 0:
            ...
            draw_robot_ui(
                "請在下方輸入框寫下名字吧：", show_input=True, input_val=user_text
            )
        elif chatbot_step == 2:
            draw_robot_ui(
                f"{player_name}，你好呀！很高興認識你！\n接下來，請讓我為你說明一下遊戲規則。\n準備好了就按 Enter 吧！"
            )
        elif chatbot_step == 3:
            draw_robot_ui(
    ...
    # ================= MENU =================
        ...
        for name, btn in menu_buttons.items():
            ...
            if pressed_button_name == name and (
                current_time - button_flash_timer < FLASH_DURATION
            ):
                current_img = img_p
            # 否則，維持原本的滑鼠懸停判定
    ...
    # ================= 選擇關卡 =================
        ...
        # 延遲切換進入遊戲關卡
        if pressed_level_index is not None and (
            current_time - button_flash_timer >= FLASH_DURATION
        ):
    ...
    # ================= GAME =================
        ...
        pygame.draw.rect(
            screen, box_color, input_box_rect, 3, border_radius=8
        )  # 3像素寬，圓角8
        ...
        screen.blit(
            font.render(f"Level {level}-{stage_index+1}", True, (255, 255, 255)),
            (50, 20),
        )
        screen.blit(font.render(f"得分：{score}", True, (255, 255, 255)), (50, 60))
        screen.blit(
            font.render(
                f"剩餘時間：{max(0, int(60 - elapsed))}秒", True, (255, 255, 0)
            ),
            (750, 20),
        )  # 顯示時間(倒數)
    ...
    # ================= GAME OVER =================
        ...
        stats1 = font.render(
            f"每分鐘所打的單字數(WPM): {final_wpm:.1f}", True, (255, 255, 255)
        )
        stats2 = font.render(
            f"每分鐘所打的字元數(CPM): {final_cpm:.1f}", True, (255, 255, 255)
        )
        ...
        pygame.draw.rect(screen, (30, 30, 35), bubble_rect, border_radius=15)
        pygame.draw.rect(
            screen, (255, 50, 50), bubble_rect, 3, border_radius=15
        )  # 紅色邊框
        line1 = font.render(f"別灰心，{player_name}！再接再厲！", True, (255, 255, 255))

        line2 = font.render("以下是你的本次成績：", True, (255, 255, 255))
    ...
    # ================= result =================
        ...
        pygame.draw.rect(screen, (30, 30, 35), bubble_rect, border_radius=15)
        pygame.draw.rect(
            screen, (0, 255, 100), bubble_rect, 3, border_radius=15
        )  # 綠色邊框

        line1 = font.render(
            f"太棒了，{player_name}！你成功通關了！", True, (255, 255, 255)
        )
        ...
        # 讀取算好的靜態數據
        wpm_text = font.render(
            f"每分鐘所打的單字數(WPM): {final_wpm:.1f}", True, (255, 255, 255)
        )
        cpm_text = font.render(
            f"每分鐘所打的字元數(CPM): {final_cpm:.1f}", True, (255, 255, 255)
        )
